...
export default function AddTransactionScreen({ route, navigation }: any) {
  ...
  const { businessId, businessName, transactionType } = route.params;
  ...
  const handleSaveTransaction = async () => {
    ...
    try {
      ...
      Alert.alert(
        'Success',
        `${isCredit ? 'Credit taken' : 'Payment made'} successfully`,
        [
          {
            text: 'OK',
        ...
        <View style={styles.centerSection}>
          ...
          {amount && parseFloat(amount) > 0 && (
            <Text style={[styles.amountHint, { color: Colors.textTertiary }]}>
              {isCredit ? 'You will owe' : 'Paying back'}
            </Text>
          )}
        </View>
